# Unstop Case Competition Radar & Team Matching Engine
...
Curated specifically for **Undergraduate Eligibility**, removing all MBA-only, PG-only, school-only, and expired listings.
...
- **Circuit Intelligence:** Automatically classifies competitions into:
...
- **Teammate Finder Bridge:** One-click integration that pre-fills squad requirements (title, organizer, member slots) for team formation.
- **Zero External Icon Dependencies:** 100% inline SVGs to guarantee zero bundler chunk collisions.
- **Local Persistence:** Bookmarks saved across sessions via `localStorage`.
...
│   ├── App.jsx               # App shell with theme toggle & teammate route handling
